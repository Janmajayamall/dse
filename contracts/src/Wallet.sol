...
contract Wallet {
    ...
    uint256 public immutable indexValue;
    // Buffer time for burn
    uint256 public immutable burnBuffer;
    ...
    // signature.
    uint256 constant invDisadvantageBuffer = 1 hours;
    ...
    function burnPendingAmount() internal {
        uint256 _pendingAmount = pendingBurnAmount;
        if (_pendingAmount != 0) {
    ...
    function recoverSigner(uint256 sIndex, bytes32 message)
        internal
        pure
        returns (address)
    {
    ...
    function isValidCommitRange(
        ...
        if (epoch != currentEpoch || block.timestamp >= epochExpiresBy)
            return false;

        // FIXME: this does not work rn
        ...
        uint256 indexMax = reserves / (2 * indexValue);
        if (indexMax <= eIndex) {
            return true;
        } else {
            return false;
    ...
    function redeemdType2() public {
        ...
        for (uint256 i = 0; i < uint256(count); i++) {
            uint32 index;
            uint32 u;
            uint256 byteOffset = 26 + (139 * i); // 139 = 4 + 4 + 65 + 65
            assembly {
            ...
            // check that index for the epoch hasn't been claimed
            // nor pending burn
            bytes32 claimIdentifier = keccak256(
                abi.encodePacked(index, _currentEpoch)
            );
            if (
                burnBufferTime[claimIdentifier] != 0 ||
                claimed[claimIdentifier] == true
    ...
    function burnCommits() public {
        uint256 _burnBuffer = burnBuffer;

        // Can't call burn after epochExpiry - burnBuffer - invDisadvantageBuffer.
        if (
            block.timestamp >=
            epochExpiresBy - (_burnBuffer + invDisadvantageBuffer)
        ) revert();
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            uint32 u;
            uint8 _type;
            uint256 byteOffset = 26 + (74 * i); // 74 = 4 + 4 + 1 + 65
            assembly {
                index := shr(224, calldataload(byteOffset))
                u := shr(224, calldataload(add(byteOffset, 4)))
    ...
    function challengeBurnCommits(
        ...
        for (uint256 i = 0; i < commitHash.length; i++) {
            ...
            if (
                ecrecover(burnBufferIBlob[commitHash[i]], v[i], r[i], s[i]) !=
                burnBufferIAddress[commitHash[i]]
            ) revert();
